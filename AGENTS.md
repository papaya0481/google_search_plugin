...
- Extract 默认使用 `basic`、原问题作为 `query`、每来源 3 个相关 chunk，以决策轮为上限。
- subagent 默认最多 4 个成功决策轮。LLM 失败、超时、空响应和响应解析失败使用重试预算，失败后重试 2 次，不消耗决策轮。
- Extract 超时、限流、服务端错误和网络异常共享同一重试预算，不额外消耗决策轮；耗尽后将明确错误交回 subagent。
